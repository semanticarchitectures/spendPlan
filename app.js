  ...
  chartSpendingDonut = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (ctx) => ` ${ctx.label}: ${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(ctx.parsed)}`
          }
        }
      }
    }
  ...
  chartScenario = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (ctx) => ` ${ctx.dataset.label}: ${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(ctx.parsed.y)}`
          }
        }
      },
      scales: {
...
function initModals() {
  document.getElementById('form-budget').addEventListener('submit', (e) => {
    ...
    const actual = actualProvided ? toMonthly(actualRaw, frequency) : expected;

    if (id) {
      const idx = state.budget.findIndex(b => b.id === id);
      if (idx !== -1) {
        const prev = state.budget[idx];
        const next = Object.assign({}, prev, { name, type, expected, frequency });
        // If the user typed an actual, that becomes a manual override; otherwise keep
        // whatever the ledger derived (or the previous manual value).
        if (actualProvided && actualRaw !== toDisplay(prev.actual, frequency)) {
          next.actual = actual; next.actualSource = 'manual'; delete next.actualTxCount;
        } else if (!actualProvided) {
          next.actual = prev.actualSource === 'transactions' ? prev.actual : expected;
        }
